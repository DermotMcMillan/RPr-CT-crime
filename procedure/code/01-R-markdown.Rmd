...
packages <- c("tidyverse", "here", "sf", "tidycensus", "tmap", "tigris", "broom", "kableExtra")
...
ct_subdivs_calc <- ct_subdivs %>%
  ...
  mutate(age = (age_20mE + age_20fE + age_21mE + age_21fE + age_22_24mE + age_22_24fE+ age_25_29mE + age_25_29fE + age_30_34mE + age_30_34fE)/total_populationE,
         ...
         education = (education_totalE - (education_assocE + education_baE + education_maE + education_proE + education_phdE))/ education_totalE,
         ...
         res_mobility = (moved_within_countyE + moved_within_stateE)/ moved_totalE,
         
         household_type = (lone_parent_families_mE + lone_parent_families_fE)/households_totalE,
         
         pop_density = total_populationE/ st_area(geom),
         GEOID = as.integer(GEOID)) %>%
  
  select(GEOID, NAME, age, education, median_income, unemployment_rate, rent_rate,multi_unit_rate, res_mobility, pop_density)
```
...
cols <- c("age", "education", "median_income", "unemployment_rate", "rent_rate", "multi_unit_rate", "res_mobility")

ct_subdivs_calc %>% 
  st_drop_geometry() %>%
  summarise_at(cols, list(min = min, max = max, median = median), na.rm=T)
```
